...
/datum/component/label/proc/OnAttackby(datum/source, obj/item/attacker, mob/user)
	...
	if(do_after(user, 5 SECONDS, target = source))
		...
		discarded_label.desc += span_notice("<br>Residue on the back looks like it might've belonged to [source].") // the description retains the label text, which it shouldn't, but someone can fix that like 5 years from now or something.
